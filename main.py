...
# ============= CONFIGURATION (INTEGRATED FROM config.json) =============
...
# Group sources for easier access
SOURCES = {
...
# ============= DECK PATHS PERSISTENCE =============
# Instead of config.json, we'll use a small pickle file just for deck paths
DECK_PATHS_FILE = os.path.join(BASE_DIR, "deck_paths.pkl")

def load_deck_paths():
    """Load saved deck paths from pickle file"""
    try:
        with open(DECK_PATHS_FILE, 'rb') as f:
            return pickle.load(f)
    except:
        # Default empty paths if file doesn't exist or can't be loaded
        return {
            "player1": "",
            "player2": ""
        }
...
def save_deck_paths(player1_path, player2_path):
    """Save deck paths to pickle file"""
    deck_paths = {
...
class OBSController:
    ...
    def setup_ui(self):
        ...
        # Create UI sections - Fixed estadio section to be closer to dropdown
        current_row = 1
        ttk.Label(
            self.master, 
            text="Estádio", 
        ...
        # Player sections with deck selection
        # Player 1 section
        ttk.Label(
            self.master, 
            text="Jogador 1", 
        ...
        # Deck selection buttons in a separate row for symmetry
        ttk.Button(
            self.master,
            text="Selecionar",
    ...
    def select_deck_folder(self, player):
        """Open folder dialog to select a deck folder"""
        ...
        if folder:
            ...
            # Update dropdowns for this player
    ...
    def update_player_dropdowns(self, player):
        """Update dropdowns for a specific player"""
        for source in SOURCES[player]:
            dropdown = self.dropdowns.get(source)
            if dropdown and "prize" not in source.lower():  # Don't update prize dropdowns
    ...
    def create_dropdown(self, source, row, label_col, dropdown_col):
        """Create a label and dropdown for a source"""
        ttk.Label(
            self.master, 
            text=source
    ...
    def get_images_for_source(self, source):
        """Get the appropriate image list for a source"""
        # First check if it's a prize source
        if "prize" in source.lower():
            return self.images["prizes"]
        # Then check if it's a stadium
        elif source.lower() == "estadio":
            return self.images["estadio"]
        # Then check if it belongs to a player
        elif source in SOURCES["player1"]:
            # Use player1's deck if available, otherwise use general pokemon folder
            ...
            # Use player2's deck if available, otherwise use general pokemon folder
            if self.deck_paths["player2"].get() and self.images["player2_deck"]:
                return self.images["player2_deck"]
            return self.images["pokemon"]
        # Default to pokemon folder
        else:
            return self.images["pokemon"]
    
    def get_folder_for_source(self, source):
        """Get the appropriate folder for a source"""
        # First check if it's a prize source
        if "prize" in source.lower():
    ...
    def refresh_images(self):
        """Refresh image lists from folders"""
        # Update image lists
        self.images = {
            "pokemon": get_image_files(FOLDERS["pokemon"]),
            "estadio": get_image_files(FOLDERS["estadio"]),
    ...
    def load_obs_settings(self):
        """Load current settings from OBS"""
        if not self.ws:
